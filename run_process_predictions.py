...
def get_score_from_metric(
    ...
    if bucket_metadata_dict:
        corpus_score = metric.compute_from_dataset(
            output_dataset, bucket_metadata_dict=bucket_metadata_dict, seed=seed
        )
...
def _instantiate_output_dataset_instances_queue(output_dataset_cfg, num_workers):
    ...
    for _ in range(num_workers):
        output_dataset_instances_queue.put(
            hydra.utils.instantiate(output_dataset_cfg, _recursive_=False)
        )
...
def get_bootstrap_run_scores(
    ...
    if output_dataset_instances_queue is None:
        output_dataset_instances_queue = _instantiate_output_dataset_instances_queue(
            cfg.output_dataset, num_workers
        )
...
def run_process_predictions(
    ...
    # api = wandb.Api()
    run = wandb.init()
    cfg.wandb_run_path = run.path

    (
        wandb_run_config,
        wandb_run_hydra_config,
        abs_exp_dir,
    ) = evaluation_helpers.fetch_experiment_from_wandb(
    ...
    linearization_class_id = wandb_run_hydra_config["datamodule"].get(
        "linearization_class_id", None
    )
    if cfg.get("linearization_class_id", None):
        ...
        log.info(
            f"Overriding the linearization class id with the value from the config `{linearization_class_id}`"
        )
    elif linearization_class_id is None:
        # Left for backward compatibility
        log.info(
            "Linearization class ID not specified. Using the default one `fully_expanded_et`"
        )
    ...
    for metric_id, metric in metrics.items():
        if metric.name in results and len(results[metric.name]) > 0:
            log.info(
                f"Skipped -- {metric.name} -- as it is already present in the results json."
            )
        else:
            results[metric.name] = {}
        ...
        if cfg.get("compute_macro_metrics", False):
            if macro_metadata_dict is None:
                macro_metadata_dict = (
                    evaluation_helpers.get_macro_metrics_computation_metadata(
            ...
            if metric_name in results and len(results[metric_name]) > 0:
                log.info(
                    f"Skipped -- {metric_name} -- as it is already present in the results json."
                )
            else:
                results[metric_name] = {}
        ...
        if cfg.get("compute_rel_centric_buckets_metrics", False):
            if rel_centric_bucket_metadata_dict is None:
                ...
                rel_centric_bucket_metadata_dict = evaluation_helpers.get_rel_centric_bucket_metrics_computation_metadata(
            ...
            dataset_id = evaluation_helpers.get_dataset_id(
                dataset_cfg=train_cfg, from_cfg=True
            )
            if metric_name in results and dataset_id in results[metric_name]:
                log.info(
                    f"Skipped -- {metric_name} [{dataset_id}] -- as it is already present in the results json."
                )
            else:
                ...
                results[metric_name][dataset_id]["score"] = metric.compute_from_dataset(
                    output_dataset,
                    bucket_metadata_dict=rel_centric_bucket_metadata_dict,
                )
                ...
                results[metric_name][dataset_id]["metadata"].update(
                    serializable_metadata
                )

            score = Results._get_score(
                results, metric_name, dataset_id=dataset_id, per_bucket=False
            )
        ...
        if cfg.get("compute_num_target_triplets_centric_buckets_metrics", False):
            if dp_centric_bucket_metadata_dict is None:
                dp_centric_bucket_metadata_dict = evaluation_helpers.get_num_target_triplets_centric_bucket_metrics_computation_metadata(
                    output_dataset=output_dataset,
                )
    ...
    if cfg.get("bootstrap_n", None):
        ...
        for metric_name in results:
            ...
            if output_dataset_instances_queue is None:
                output_dataset_instances_queue = (
                    _instantiate_output_dataset_instances_queue(
            ...
            lower, mean_perc_based, upper = evaluation_helpers.get_percentile_based_ci(
...
@hydra.main(
    version_base="1.2",
    config_path="configs/hydra_conf",
    config_name="process_predictions_root",
)
