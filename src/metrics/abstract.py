...
class ELAbstractTorchMetric(Metric, ABC):
    full_state_update = True
    ...
    @staticmethod
    def _process_test_sample(pred_entity: str, target_entity: set):
        num_matched = target_entity == pred_entity
        num_predicted = 1
        num_target = 1
    ...
    @classmethod
    def _process_lists(
        cls, preds: List[str], targets: List[str]
    ) -> Tuple[float, float, float]:
        num_correct = 0
        num_predicted = 0
        num_target = 0
        ...
        for p, t in zip(preds, targets):
            n_matched, n_predicted, n_target = cls._process_test_sample(pred_entity=p, target_entity=t)
    ...
    def update(self, preds: List[str], targets: List[str]):
        ...
        for t, p in zip(targets, preds):
            n_matched, n_predicted, n_target = self._process_test_sample(pred_entity=p, target_entity=t)
