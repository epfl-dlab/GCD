...
from src.utils.file_io import read_jsonlines, write_jsonlines
import logging
...
def apply_ordering_heuristic_to_datapoint(dp):
    input_text = dp["text"].lower()
    entities = dp["entities"]
    entity_surfaceforms_parts = [
        entity["surfaceform"].lower().split("_") for entity in entities
    ]
    mention_start_index = [
        get_start_index_parts(input_text, sf_parts)
        for sf_parts in entity_surfaceforms_parts
    ]
    ...
    # apply_ordering
    sub_obj_sf_pairs = [
        (triplet["subject"]["surfaceform"], triplet["object"]["surfaceform"])
        for triplet in dp["triplets"]
    ]
    sub_obj_start_idx_pairs = np.array(
        [
            (_get_start_index(dp, sub_sf), _get_start_index(dp, obj_sf))
            for sub_sf, obj_sf in sub_obj_sf_pairs
        ]
    ).T
    sub_ent_name = [triplet["subject"]["surfaceform"] for triplet in dp["triplets"]]
    ordered_indices = np.lexsort(
        (sub_obj_start_idx_pairs[1, :], sub_ent_name, sub_obj_start_idx_pairs[0, :])
    )
...
def _apply_ordering_heuristics_info(data, num_examples_to_show):
    ...
    mention_start_indices = np.array(
        [entity["mention_start_index"] for dp in data for entity in dp["entities"]]
    )
...
def apply_ordering_heuristic(
    data_dir, split, output_split, verbose, num_examples_to_show, output_dir=None
):
