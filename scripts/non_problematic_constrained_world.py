...
import src.constrained_generation.trie_constrained_generation_utils as cg_utils
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--constrained_worlds_dir", type=str, default=constrained_worlds_dir
    )
    parser.add_argument(
        "--path_to_relid2name_mapping", type=str, default=path_to_relid2name_mapping
    )
    parser.add_argument(
        "--path_to_entid2name_mapping", type=str, default=path_to_entid2name_mapping
    )
    ...
    parser.add_argument(
        "--tokenizer_short_name", type=str, default=tokenizer_short_name
    )

    parser.add_argument(
        "--linearization_class_id", type=str, default="subject_collapsed"
    )
    ...
    non_problematic_entity_ids = cg_utils.get_ids_for_names(
        non_problematic_names, path_to_entid2name_mapping
    )

    # Create a new constrained world with the non-problematic names only
    output_constrained_world_id = (
        f"{args.constrained_world_id}_{args.tokenizer_short_name}_tokenizeable"
    )
    print(
        "Saving the new constrained world to",
        os.path.join(args.constrained_worlds_dir, output_constrained_world_id),
    )
    cg_utils.write_constrained_world(
        args.constrained_worlds_dir,
        output_constrained_world_id,
        non_problematic_entity_ids,
        relation_ids,
    )
